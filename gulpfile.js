...
gulp.task('compile', ['clean'], function() {
  return browserify('lib/bespoke-PLUGIN_NAME.js', { standalone: 'bespoke.plugins.PLUGIN_NAME' }).bundle()
    .pipe(source('bespoke-PLUGIN_NAME.js'))
    .pipe(buffer())
    .pipe(header([
      '/*!',
      ' * <%= name %> v<%= version %>',
    ...
    .pipe(gulp.dest('dist'))
    .pipe(rename('bespoke-PLUGIN_NAME.min.js'))
    .pipe(uglify())
    .pipe(header([
      '/*! <%= name %> v<%= version %> ',
...
gulp.task('compile:demo:js', ['compile'], function() {
  // TODO list of plugins may need to be updated depending on what is needed for the demo
  return gulp.src([
